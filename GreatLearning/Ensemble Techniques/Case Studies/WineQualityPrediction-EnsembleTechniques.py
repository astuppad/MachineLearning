# -*- coding: utf-8 -*-
"""
Created on Sat Jan 25 08:07:43 2020
...
tempResultDf = pd.DataFrame({'Method': ['Random Forest'], 'accuracy': acc_RF})
resultsDf = pd.concat([resultsDf, tempResultDf])
resultsDf = resultsDf[['Method', 'accuracy']]
...
predict_AB = abcl.predict(X_test) 
acc_AB = accuracy_score(y_test, predict_AB)

tempResultDf = pd.DataFrame({'Method': ['AdaBoosting'], 'accuracy': [acc_AB]})
resultsDf = pd.concat([resultsDf, tempResultDf])
resultsDf = resultsDf[['Method', 'accuracy']]
resultsDf
...
predict_BAG = bagcl.predict(X_test)
acc_BAG = accuracy_score(y_test, predict_BAG)

tempResultDf = pd.DataFrame({'Method': ['Bagging'], 'accuracy': acc_BAG})
resultsDf = pd.concat([resultsDf, tempResultDf])
resultsDf = resultsDf[['Method', 'accuracy']]
resultsDf
...
predict_GB = gbcl.predict(X_test)
acc_GB = accuracy_score(y_test, predict_GB)

tempResultDf = pd.DataFrame({'Method': ['Gradient Boosting'], 'accuracy': acc_GB})
resultsDf = pd.concat([resultsDf, tempResultDf])
resultsDf = resultsDf[['Method', 'accuracy']]
